...
export function ToolStudio() {
  ...
  const selectTool = (id: ToolId) => {
    ...
    gsap.to('.studio-stage', { autoAlpha: 0, y: 10, duration: 0.18, onComplete: () => {
      window.history.replaceState(null, '', `#${id}`)
      setActiveId(id)
      requestAnimationFrame(() => {
        gsap.fromTo('.studio-stage', { autoAlpha: 0, y: 10 }, { autoAlpha: 1, y: 0, duration: 0.42 })
  ...
  return <section ref={scope} className="tool-studio" id="studio">
    ...
    <div className="studio-stage" style={{ '--active-accent': activeTool.accent } as React.CSSProperties}>
      <div className="stage-bar"><span><i /> WEB INTERACTIVE DEMO</span><div><button type="button" onClick={() => document.getElementById('workflow')?.scrollIntoView({ behavior: 'smooth' })}>使用流程 <ExternalLink /></button></div></div>
